...
**Switch Axe (Uncommon+)**: overlay `applySwitchAxeOverlay` — AEs indicador **Axe Mode** (default) / **Sword Mode**; sin Activity Sword; golpe Sword = **Phial Discharge** (Attack `2d6` + phial, consume 1; gated por `flags.world.sa.installedPhial`); **Switch Phial** utility 1/LR (diálogo de Phials desbloqueados → instala uno); **Fluid Morph** BA alterna modos vía ItemMacro; **Kinetic** recupera uses en hit Axe; 0 charges en Sword aborta y revierte a Axe. **ZSD** (Uncommon+): reemplaza un ataque de la Attack action (1/turno); Reach 10 ft; dump all Phial Charges (+1d6→1d8→1d10 Upgrade I/II); sin Advantage; PB usos / Short Rest; recoil → Axe a 0. Phials Uncommon: **Power** (+1d8 Slashing) + **Acid / Cold / Fire / Lightning** (+1d6 cada uno — menos dados, tipo elemental). **Rare:** Expanded Gauge I (`uses` max 7), **ZSD Splash** Save DEX radio 5 ft (trigger desde ZSD), Phials **Exhaust** (AE −10 walk) / **Poison** (Save CON). Macro: `switch-axe-kinetic.macro.ts`. Ejemplos: `weapons/switch-axe/fvtt-Item-switch-axe-uncommon.json`, `weapons/switch-axe/fvtt-Item-switch-axe-rare.json`.

**Charge Blade (Uncommon+)**: overlay `applyChargeBladeOverlay` — AEs indicador **Sword & Shield Mode** (default) / **Axe Mode**; Attacks por modo con **`@mod`** en damage parts; **Switch Mode** BA + deshabilita Integrated Shield en Axe + swap mastery Sap/Cleave; **Elemental Attunement** utility 1/SR (diálogo Acid/Cold/Fire/Lightning; sin elemento por defecto — actualiza `flags.world.chargeBlade.elementalType` y types de Eruption/Discharge/AED); **Phial Charges** recupera 1 en hit Sword; **Elemental Guard** (activity **Guard Point**): Reaction + Guard Dice a la CA (d4→d6→d8) + **Eruption** al fallar el ataque; **Elemental Discharge** (diálogo Yes/No tras hit Axe, Uncommon+). **Rare:** **AED** Activity única (reemplaza un ataque de la Attack action; PB/LR; diálogo de cargas 1…available; dados 1d6→1d8→1d10 SAED). Macro: `charge-blade.macro.ts`. Ejemplos: `weapons/charge-blade/fvtt-Item-charge-blade-uncommon.json`, `weapons/charge-blade/fvtt-Item-charge-blade-rare.json`.
...
**Very Rare / Legendary (todas las armas Forge)**: los goldens `fvtt-Item-*-very-rare.json` / `*-legendary.json` se regeneran con `UPDATE_FOUNDRY_EXAMPLES=1` + `generate-vr-legendary-examples.test.ts`. Features nuevas de combate emiten Activity/AE (p. ej. Hammer Offset Smash / Big Bang, Charge Blade Charged Shield + SAED 30 ft 1d10, Dual Blades Demon Dance / Heavenly Blade Dance / Dual Mastery, Hunting Horn Infernal Melody + Songbook Mastery, Dual Repeaters Tactical Modifications / Perfect Empowerment / Capacitive Frame, SnS True Perfect Rush: Stun, Bow True Dragonpiercer 4d6 + ignore cover/piercing resistance). Los `upgrade_scaler` no crean Activity extra: mutan la del padre (dados, alcance, uses). Overlays Midi + ItemMacro viven en `weapon-forge-*-export.ts` / `*.macro.ts` con espejo en `public/data/scripts/weapons-resources/`.
...
**Ruta**: `/item-forge`
**Fuente de datos**: `public/data/raintdm-items/` (`manifest.json` + un JSON por categoría, p. ej. `magazines.json`, `traps.json`, `potions.json`). Fetch en runtime; sin IndexedDB ni editor.
...
Extiende `MHItem` (`name`, `source`, `type`, `typeLabel`, `rarity`, `valueCp`, `weight`, `entries[]`) con `raintdm?` (`author`, `kind`, `magazineKey`, `trapKey`, `potionKey`, `bombKey`, `chargesPerMagazine`, `damageType`, `baseWeapon`) mapeado desde `_raintdm`, y `crafting?` (`tool`, `item1`, `item2`, `dc`, `quantity?`) desde el campo top-level `crafting` del JSON.

- Tipo `MHMAG` → label **Magazine (Repeaters)**. Tipo `MHTRAP` → **Traps**. Tipo `MHPOTION` → **Potions & Consumables**. Tipo `MHBOMB` → **Bombs**. Tipos desconocidos → **Misc**.
...
- v3: AGMH potions / consumables in Items Forge (`potions.json` → `items-forge/potions/`). **Buff AE**: Adamant Pill, Armorskin / Mega, Dash / Mega Dash, Demondrug / Mega, Nutrients / Mega, Frozen Berry, Might Pill (STR; exhaustion on end is manual), Air / Mega Air Philter, Psychoserum, Immunizer, Antiseptic Stone. **Heal activities**: Ancient Potion (45 + clear exhaustion), Max Potion (70), Herbal Medicine (1d4 + poison-save AE), Lifepowder / Dust of Life (AoE heal 30 ft, up to 8 allies). **Other**: Energy Drink (exhaustion −1 via AE), Power Juice (concentration + Haste-like AE; extra Attack action not automated). Build: `build-potions.mjs` resolves Foundry core icons via `scripts/lib/foundry-icons.mjs`.
- v4: AGMH bombs (`bombs.json` → `items-forge/bombs/`). **AoE point + radius (save)**: Flash Bomb (60 ft / 5-ft radius / Con DC 13 → Blinded), Sonic Bomb (60 ft / 30-ft / Con DC 13 → Deafened), Sm/Lg Barrel-Bomb + Bounce Bomb (Ignite, Dex save, fire damage half). **Template**: Smoke Bomb (40 ft / 20-ft heavily obscured). **Poison Smoke**: Throw template + Use Gas save (Con DC 13 → Poisoned). **Attack (as book)**: Dung Bomb, Tranq Bomb (5d8 threshold roll), Paintball (Paint Marked). Flash is intentionally **not** a creature attack — AGtMH throws at a point.
- **Foundry module**: magazines already live in the **Weapon Resources** pack. Hunter traps + potions ship in the **Items Forge** pack (`public/data/foundry-jsons-example/items-forge/`, regenerate with `node …/build-items-forge.mjs` then `pnpm build:foundry-module`). Set Trap / Retrieve are Item Macros; canvas trigger, camouflage notices, and 1-hour expiry run from `scripts/hunter-traps.js`. Combo Crafting can import Trap Tool / Pitfall / Shock from that pack by name. **Conditions & Diseases** pack (`foundry-jsons-example/conditions/`) registers Amellwind blights/diseases as token HUD statuses with Active Effect automation via `scripts/amellwind-conditions.js`.
