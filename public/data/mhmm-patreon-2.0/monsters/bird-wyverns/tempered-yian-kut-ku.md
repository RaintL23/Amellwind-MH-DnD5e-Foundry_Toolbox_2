---
name: Tempered Yian Kut-Ku
slug: tempered-yian-kut-ku
group: Bird Wyverns
cr: "10"
pdfPage: 44
inGithubJson: false
source: MHMM-Patreon-2.0
size: Large
creatureType: wyvern (bird)
alignment: unaligned
ac: 17
acFrom: natural armor
hp: 153
hpFormula: "18d10 + 54"
speed: "40 ft., fly 40 ft."
lootRolls: 3
qa: missing-bio
---

# Tempered Yian Kut-Ku

## Stat Block

_Large wyvern (bird), unaligned_

- **Armor Class:** 17 (natural armor)
- **Hit Points:** 153 (18d10 + 54)
- **Speed:** 40 ft., fly 40 ft.
- **Saving Throws:** Str +7, Con +7, Wis +6
- **Skills:** Perception +6
- **Damage Resistances:** bludgeoning, piercing, and slashing from nonmagical attacks
- **Challenge:** 10

## Traits

### Special

Damage Immunity necrotic Senses passive Perception 16 Languages - Challenge 10 (5,900 XP) Proficiency +4 Sensitive Ears. If the yian kut-ku takes thunder damage, or a spell or effect that emits a loud noise (per the description of the effect) is used within 60 feet of it, it must succeed on a DC 15 Constitution saving throw or be stunned until the start of its next turn.

## Actions

### Multiattack

The yian kut-ku makes three Peck attacks or three Mini-Fireball attacks.

### Peck

### Melee Weapon Attack

+7 to hit, reach 5 ft., one target. Hit 12 (2d8 + 3) piercing damage.

### Tail

### Melee Weapon Attack

+7 to hit, reach 5 ft., one target. Hit 14 (2d10 + 3) bludgeoning damage. If the target is a creature it must succeed on a DC 15 Strength saving throw or be pushed 5 feet away and knocked prone.

### Mini-Fireball

### Ranged Weapon Attack

+7 to hit, range 60 ft., one target. Hit 14 (4d6) fire damage.

### Fireball (Recharge 5-6)

The yian kut-ku exhales a larger fireball at a point within 60 feet of it. Each creature in a 15-foot radius sphere centered on that point must make a DC 15 Dexterity saving throw, taking 35 (10d6) fire damage on a failed save or half as much damage on a successful one.

## Legendary Actions

The yian kut-ku can take 2 legendary actions, choosing from the options below. Only one legendary action option can be used at a time and only at the end of another creature's turn. The yian kut-ku regains spent legendary actions at the start of its turn.

### Tail Attack

The yian kut-ku makes one Tail attack.

### Shovel Charge (Costs 2 Actions)

The yian kut-ku digs its beak into the ground as it moves up to its speed. During this move it can move through the spaces of other creatures without provoking opportunity attacks. Each creature the yian kut-ku moves through or is 10 or less feet underground in its path must succeed on a DC 15 Dexterity saving throw or take 16 (2d12 + 3) bludgeoning damage and be knocked prone. 38

## Loot

**Carves/Capture rolls:** 3

| Carve | Capture | Material         | Slots |
| ----- | ------- | ---------------- | ----- |
| 1-6   | 1-3     | Kut-Ku Scale+    | A, W  |
| 7-9   | 4-9     | Kut-Ku Carapace+ | A, W  |
| 10-11 | —       | Kut-Ku Wing      | A, W  |
| 12-13 | 10      | Giant Beak+      | A, W  |
| —     | 11-14   | Lg Monster Bone  | O     |
| —     | 15-16   | Y.Inferno Sac    | A, W  |
| 14-19 | —       | Kut-Ku Ear+      | A, W  |
| 20    | 17-20   | Kut-Ku Gem       | A, W  |

## Materials (Runes)

These map to `Rune` entities (`name`, `carveChance`, `captureChance`, `slots`, `armorEffect`, `weaponEffect`).

### Kut-Ku Scale+

- **Slots:** A, W
- **Carve:** 1-6
- **Capture:** 1-3
- **Armor effect:** While attuned to this armor your gloves create small puffs of pink feathers when you clap.
- **Weapon effect:** While attuned to this weapon, you can use an action to mark any surface with a colored mark. The mark can be any color of your choosing and fade away after 24 hours.

### Kut-Ku Carapace+

- **Slots:** A, W
- **Carve:** 7-9
- **Capture:** 4-9
- **Armor effect:** Entomologist+. When you capture an insect with a bug net, you instead gather 1d4.
- **Weapon effect:** (Spellcaster Only) This weapon has 4 runes. While holding it, you can use an action to expend 1 or more of its runes to cast the scorching ray spell from it using your ranged spell attack modifier. For 1 rune, you cast the 2nd-level version of the spell. You can increase the spell slot level by one for each additional rune you expend. This weapon regains 1d4 expended runes daily at dawn. If you expend the weapon's last rune, roll a d20. On a 1, the runes cannot recharge for a week.

### Kut-Ku Wing

- **Slots:** A, W
- **Carve:** 10-11
- **Capture:** —
- **Armor effect:** Recovery Speed. Whenever you roll a Hit Die to regain hit points, double the number of hit points it restores.
- **Weapon effect:** Charger. When you use your action to Dash, you can use a bonus action to shove a creature.

### Giant Beak+

- **Slots:** A, W
- **Carve:** 12-13
- **Capture:** 10
- **Armor effect:** Constitution. While you are attuned to this armor, you can reroll a Constitution saving throw that you fail. If you do so, you must use the new roll, and you can't use this property again until you finish a long rest.
- **Weapon effect:** Critical Eye. Your weapon attacks critical hit range is increased by 1.

### Lg Monster Bone

- **Slots:** O
- **Carve:** —
- **Capture:** 11-14
- **Other effect:** Very rare armor upgrade material.

### Y.Inferno Sac

- **Slots:** A, W
- **Carve:** —
- **Capture:** 15-16
- **Armor effect:** While you are attuned to this armor, you can use a bonus action to speak its command word and exhale fire at a target within 30 feet of you. The target must make a DC 16 Dexterity saving throw, taking 4d6 fire damage on a failed save, or half as much damage on a successful one. You can use this property a number of times equal to half your proficiency bonus, regaining all expended uses when you finish a long rest.
- **Weapon effect:** Your fire spells deal an extra 1d6 fire damage.

### Kut-Ku Ear+

- **Slots:** A, W
- **Carve:** 14-19
- **Capture:** —
- **Armor effect:** When you make a skill check while wearing this armor, you can use your reaction to gain advantage on it. You can use this property a number of times equal to half your proficiency bonus (minimum 2), regaining all expended uses when you finish a long rest.
- **Weapon effect:** While touching this weapon you can speak its command phrase: "The limits of my language are the limits of my world" to gain proficiency in any two languages of your choice for 24 hours. Once you use this property, you can't use it again until the next day.

### Kut-Ku Gem

- **Slots:** A, W
- **Carve:** 20
- **Capture:** 17-20
- **Armor effect:** You are immune to fire damage while you wear this armor.
- **Weapon effect:** While attuned to this weapon you can use an action to speak its command word to cast the polymorph spell, but you can only target yourself and you can only transform into a regular yian kut-ku. Once you use this property, you can't use it again until you finish a long rest.

## QA flags

`missing-bio`
