...
// JSON exports in `public/data/foundry-jsons-example` and client engines in
// `public/data/scripts`.
//
...
/** Foundry runtime macros / engines (Item Macros, sync engines, client bootstrap sources). */
const SCRIPTS_DIR = path.join(ROOT, "public", "data", "scripts");
...
const PACKS = [
  ...
  { name: "conditions", docType: "Item", rootDir: path.join(SRC_DIR, "conditions") },
];
...
/**
 * dnd5e traits.dr/di/dv Active Effect values must be bare damage-type keys
 * (e.g. "lightning"). Prose like "lightning damage" is treated as custom and
 * breaks resistance/immunity automation.
 */
function sanitizeDamageTraitChanges(doc) {
  const walk = (node) => {
    ...
    if (Array.isArray(node.changes)) {
      for (const ch of node.changes) {
        ...
        if (!/^system\.traits\.d[riv]\.value$/.test(String(ch.key ?? ""))) continue;
        if (typeof ch.value !== "string") continue;
        const cleaned = ch.value.trim().replace(/\s+damage$/i, "");
        if (cleaned !== ch.value) ch.value = cleaned;
      }
    }
    if (Array.isArray(node.effects)) node.effects.forEach(walk);
...
function buildPackDocs({ name, docType, rootDir }) {
  ...
  for (const file of files) {
    ...
    const doc = sanitizeDamageTraitChanges(rewriteModuleAssets(raw));
    doc._id = id;
...
function buildResourceNodeClientScript() {
  const enginePath = path.join(SCRIPTS_DIR, "resource-node", "resource-node-sync-engine.js");
...
function buildHiddenDetectionClientScript() {
  const enginePath = path.join(SCRIPTS_DIR, "hidden-detect", "hidden-detection-sync-engine.js");
  if (!fs.existsSync(enginePath)) {
...
function buildFelyneCookClientScript() {
  const flowPath = path.join(SCRIPTS_DIR, "cooking-features", "felyne-cook-player-flow.fragment.js");
  const enginePath = path.join(SCRIPTS_DIR, "cooking-features", "felyne-cook-sync-engine.js");
...
/**
 * Shared helper: request saves on connected player owners (socketlib).
 * Must load before alatreon / dire-miralis / hunter-traps client scripts.
 */
function buildPlayerSaveRollsClientScript() {
  const srcPath = path.join(SCRIPTS_DIR, "shared", "player-save-rolls.js");
  if (!fs.existsSync(srcPath)) {
    console.warn("  ! player-save-rolls missing — client script skipped");
    return false;
  }
  ...
  const scriptsDir = path.join(MODULE_DIR, "scripts");
  fs.mkdirSync(scriptsDir, { recursive: true });
  fs.writeFileSync(path.join(scriptsDir, "player-save-rolls.js"), `${client}\n`);
  console.log('  ok client script "scripts/player-save-rolls.js"');
  return true;
}

/**
 * Rune runtime: weapon-type equip gate cleanup, Muse sync on move/rest,
 * Hunting Horn ↔ rune effects (Inspiring Melody / Lord's Favor).
 */
function buildRuneRuntimeClientScript() {
  const hooksPath = path.join(SCRIPTS_DIR, "runes", "rune-runtime-hooks.js");
  const hhEffectsPath = path.join(SCRIPTS_DIR, "runes", "hunting-horn-rune-effects.js");
  if (!fs.existsSync(hooksPath) || !fs.existsSync(hhEffectsPath)) {
    console.warn("  ! rune runtime scripts missing — client script skipped");
    return false;
  }
  const hooks = fs.readFileSync(hooksPath, "utf8").replace(/\r\n/g, "\n").trim();
  const hh = fs.readFileSync(hhEffectsPath, "utf8").replace(/\r\n/g, "\n").trim();
  const client = [
    "/**",
  ...
  const scriptsDir = path.join(MODULE_DIR, "scripts");
  fs.mkdirSync(scriptsDir, { recursive: true });
  fs.writeFileSync(path.join(scriptsDir, "rune-runtime.js"), `${client}\n`);
  console.log('  ok client script "scripts/rune-runtime.js"');
  return true;
}
...
function buildDireMiralisClientScript() {
  const enginePath = path.join(SCRIPTS_DIR, "monsters", "dire-miralis-engine.js");
  if (!fs.existsSync(enginePath)) {
...
/**
 * Tempered Alatreon (MHW) boss automations (Active State, Overload, Horns,
 * Escaton, Element Burst, mythic legendary gates) must run on the GM client.
 */
function buildAlatreonClientScript() {
  const enginePath = path.join(SCRIPTS_DIR, "monsters", "alatreon-engine.js");
  if (!fs.existsSync(enginePath)) {
    console.warn("  ! alatreon engine missing — client script skipped");
    return false;
  }
  ...
  const scriptsDir = path.join(MODULE_DIR, "scripts");
  fs.mkdirSync(scriptsDir, { recursive: true });
  fs.writeFileSync(path.join(scriptsDir, "alatreon.js"), `${client}\n`);
  console.log('  ok client script "scripts/alatreon.js"');
  return true;
}
...
function buildHunterTrapsClientScript() {
  const enginePath = path.join(SCRIPTS_DIR, "items-forge", "hunter-traps-engine.js");
  if (!fs.existsSync(enginePath)) {
...
/**
 * Amellwind conditions & diseases — register CONFIG.DND5E.conditionTypes + HUD
 * statuses with Active Effect changes. Must load before boss engines that apply
 * blight statuses.
 */
async function buildConditionsClientScript() {
  const enginePath = path.join(SCRIPTS_DIR, "conditions", "amellwind-conditions-engine.js");
  const registryPath = path.join(SCRIPTS_DIR, "conditions", "amellwind-conditions-registry.mjs");
  const manifestPath = path.join(SRC_DIR, "conditions", "afflictions-manifest.json");
  if (!fs.existsSync(enginePath) || !fs.existsSync(registryPath)) {
    console.warn("  ! conditions engine/registry missing — client script skipped");
    return false;
  }

  let engine = fs.readFileSync(enginePath, "utf8").replace(/\r\n/g, "\n");
  if (!engine.includes("/* @@AMELLWIND_AFFLICTIONS@@ */")) {
    console.warn("  ! conditions engine missing @@AMELLWIND_AFFLICTIONS@@ placeholder");
    return false;
  }

  const { AMELLWIND_AFFLICTIONS } = await import(
  ...
  if (fs.existsSync(manifestPath)) {
    ...
    for (const row of manifest) itemIdBySlug.set(row.id, row.itemId);
  }

  const payload = AMELLWIND_AFFLICTIONS.map((def) => ({
    ...def,
    itemId: itemIdBySlug.get(def.id) ?? null,
  }));

  engine = engine.replace(
    /\/\* @@AMELLWIND_AFFLICTIONS@@ \*\/\s*const AFFLICTIONS = \[\];/,
    `const AFFLICTIONS = ${JSON.stringify(payload)};`,
  );
  ...
  const scriptsDir = path.join(MODULE_DIR, "scripts");
  fs.mkdirSync(scriptsDir, { recursive: true });
  fs.writeFileSync(path.join(scriptsDir, "amellwind-conditions.js"), `${client}\n`);
  console.log('  ok client script "scripts/amellwind-conditions.js"');
  return true;
}
...
async function main() {
  ...
  await buildConditionsClientScript();
  buildPlayerSaveRollsClientScript();
  buildRuneRuntimeClientScript();
  buildResourceNodeClientScript();
  buildHiddenDetectionClientScript();
  buildFelyneCookClientScript();
  buildDireMiralisClientScript();
  buildAlatreonClientScript();
