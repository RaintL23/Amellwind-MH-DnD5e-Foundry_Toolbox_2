...
- **Export** (`foundry-export/`, hook `useFoundryExport`): `buildFoundryActor(input)` ensambla `system` + `items[]` + `prototypeToken`. Sub-builders (`item.builders.ts`: clase, subclase, raza, trasfondo, dotes, armas con `midiProperties`, armadura, trinkets, loot, **spells con activities** attack/save/utility y metadata school/components/range/duration), runas embebidas vía `buildRuneFoundryItem`, `advancement.builders.ts`, `effect.builders.ts`, `midi.utils.ts`, `foundry-id.utils.ts`, `mappings.ts`. Retrato/token base64.
