...
**Weapon Forge — export Foundry VTT**: desde la lista/dialog de `/weapon-forge` se puede descargar el JSON Amellwind completo o un **Item** `weapon` standalone (Core **12.331** / dnd5e **4.4.4**) por rareza seleccionada (`weapon-forge-foundry.export.ts`). El item incluye attack **activities** (una por modo de switch si aplica; Versatile PHB usa `damage.versatile`), descripción HTML enriquecida vía `toFoundryDescriptionHtml` (`@item` / `@variantrule` / `[[/r …]]` / links 5e.tools), features agrupadas por cadenas de upgrade (`buildColumnChains`) separadas con `foundryDividerHtml()` (`<hr>`) y títulos coloreados por rareza (`foundryRarityTitleHtml`), `system.description.chat` condensado (descripción base + nombres de Features/upgrades, sin columnas de resources), `midiProperties` por activity (helper compartido `midi.utils.ts`) para Midi-QOL 12.4.x, AE pasivos tipados (p. ej. AC), y `applyItemAutomation` si existe overlay por nombre. La UI muestra `FoundryModuleRequirementsNotice` (Midi required; Plutonium/CPR/AA recommended).
...
- **Export** (`foundry-export/`, hook `useFoundryExport`): `buildFoundryActor(input)` ensambla `system` + `items[]` + `prototypeToken`. Sub-builders (`item.builders.ts`: clase, subclase, raza, trasfondo, dotes, armas con `midiProperties`, armadura, trinkets, inventario tipado, **spells con activities** attack/save/utility y metadata school/components/range/duration), runas embebidas vía `buildRuneFoundryItem`, `advancement.builders.ts`, `effect.builders.ts`, `midi.utils.ts`, `foundry-id.utils.ts`, `mappings.ts`. Retrato/token base64.
