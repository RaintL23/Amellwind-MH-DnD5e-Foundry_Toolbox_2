...
- **Automatización Midi-QoL / DAE (estilo Plutonium)** (`foundry-export/automation.data.ts` + `automation.builders.ts`): tras construir los `items[]`, `buildFoundryActor` los enriquece con `applyItemAutomation(item)`, que emula cómo el módulo *Plutonium Addon: Automation* añade comportamiento en el import. Cada overlay es un _patch_ pequeño (ActiveEffects con `changes` sobre `flags.midi-qol.*`, `flags.dae.*`, `system.traits.*`) fusionado por **nombre** de item. `automation.data.ts` es el registro estático `AUTOMATIONS` (dotes, features de clase, items mágicos y conjuros comunes) portado del repo MIT `TheGiddyLimit/plutonium-addon-automation`; `automation.builders.ts` aplica el overlay (append de effects vía `buildEffect`, deep-merge de flags de item). `transfer:true` = efecto pasivo siempre activo; `transfer:false` = efecto aplicado al usar/lanzar. Requiere Midi QoL + DAE + Times Up activos en el mundo de destino. Los bonos de items mágicos conocidos (Cloak/Ring of Protection, etc.) viven ahora aquí (antes hardcodeados como `knownItemEffects` en `effect.builders.ts`). Para ampliar cobertura: agregar entradas al registro keyeadas por nombre normalizado (lowercase).
- **Import** (`foundry-import/`, hook `useFoundryImport`): `parseFoundryActor(raw)` valida `type === "character"`, recorre `items[]` y extrae clase/subclase/raza/trasfondo, dotes, conjuros, armas, armadura/escudos, trinkets y loot, además de abilities, tamaño, alineamiento, nivel y biografía. También conserva las `properties`/tags, `rarity` y `mastery` de armas/armaduras en la estructura parseada. `applyParsed` resetea el build y hace _matching_ contra los catálogos de la app (`getClassesByName`, `getAllDndRaces`, `getListDndBackgrounds`, `getDndFeatsByName`, `getSpellsByName`, `getDndWeapons`/`getDndArmors` + homebrew Amellwind), equipa hasta dos armas/trinkets y una armadura (resto al inventario) y devuelve un resumen `{ matched, unmatched }`.
- **Snapshot sin pérdidas de choices** (`foundry-export/builder-snapshot.ts`): las selecciones opcionales del builder (fighting styles, invocations, ASI/origin, skills/tools/languages elegidos, expertise, defensas, conjuros, dotes) y el equipo exacto (con properties/tags, rarity y runas) **no** son representables en el schema plano de dnd5e, así que se serializan en un flag de módulo namespaced: `actor.flags["amellwind-toolbox"].builderSnapshot` (versionado con `BUILDER_SNAPSHOT_VERSION`). Foundry preserva los flags de módulos desconocidos, por lo que el snapshot hace round-trip app→Foundry→app sin perder choices. En la exportación `gatherBuilderSnapshot` (en `useFoundryExport`) recopila el estado; en la importación `readBuilderSnapshot(raw.flags)` lo lee y, si está presente y la versión coincide, `applyBuilderSnapshot` (en `useFoundryImport`) restaura **todas** las choices y el equipo vía los setters del builder, ignorando el _matching_ heurístico (usado solo como fallback para actores sin snapshot). Además, las descripciones de features con selección incluyen un marcador legible `Toolbox choice: selected for "…"`.
