...
**Weapon Forge — export Foundry VTT**: desde la lista/dialog de `/weapon-forge` se puede descargar el JSON Amellwind completo o un **Item** `weapon` standalone (Core **12.331** / dnd5e **4.4.4** vía `FOUNDRY_EXPORT_TARGET`) por rareza seleccionada (`weapon-forge-foundry.export.ts` + `weapon-forge-description.export.ts` / `weapon-forge-activities.export.ts`). El item incluye attack **activities** (una por modo de switch si aplica; Versatile PHB usa `damage.versatile`), descripción HTML enriquecida vía `toFoundryDescriptionHtml` (`shared/foundry/description.ts`), features agrupadas por **cadenas de upgrade** (`buildColumnChains`) en **cards HTML** al estilo PHB 2024 (`foundryFeatureCardHtml` / `foundryUpgradeBlockHtml`: borde lateral por rareza tip, lead `Bonus Action.`/`Reaction.` vía `parseFeatureUsage`, cuerpo con listas), intro + Mastery fuera de cards, `system.description.chat` condensado (`foundryChatFeatureCardHtml`: título + lead + nombres de upgrade, sin cuerpo; omite columnas de recurso), `midiProperties` por activity (`shared/foundry/midi.ts` + `compileWeaponFeatureActivities`), AE pasivos tipados (p. ej. AC), y `applyItemAutomation` si existe overlay por nombre. El catálogo Amellwind reutiliza el mismo builder (`weaponToExportCustomWeapon` → `buildWeaponFoundryItem`).

**Activities de features de combate (chain-first)**: `compileWeaponFeatureActivities` (`weapon-activity.compiler.ts`) emite **una Activity (y/o AE) por cadena de upgrade**, no por rareza/filas sueltas — excepción: template `counter_spend` (alias legacy `charge_pool_attack`) emite **Gather** (opcional) + **×N botones** (si el rango ≤5) o **una activity con consumption scaling** (rangos anchos) sobre contadores `system.uses`. Params clave: `itemUsesMax` / `ownsItemUses` / `poolStartsEmpty` / `emitGather` / `spendMin`–`spendMax` / `damageFormula` (por contador) / `advantageOnUse`. Los params de `WeaponForgeFeatureDef.automation` se **mergean** raíz→hoja a la rareza exportada (`upgrade_scaler` solo aporta deltas). Identidad estable Foundry = `chainKey` (raíz); nombre mostrado = hoja activa. `automation.enabled: false` opt-out persistente (no emite; no cae al registro global). **Fuera de alcance:** unlocks con `resourceColumn` (Phials, Coatings, Ammo, Notes, Magazines…). Registro global por nombre: `weapon-feature-automation.data.ts` (lookup exacto → `Mastery Property (X)` dinámico → `Switch Mode (…)`, con fallback quitando sufijos `Upgrade…` / prefijos True|Amped|Super). Oleada actual: masteries XPHB, gauges (start empty/full), counter_spend (Charged Slash, Shelling/Full Burst, ZSD, AED, Arcane Discharge, Zero Sum Spell), mode switch, passives AC/casting, reactions/BA claros; coatings/ammo column y algunos saves on-hit siguen parciales. Preview/export weapon tabs se etiquetan **Foundry VTT v12**. UI editable solo en Forge (`FeatureAutomationEditor` en acordeón + checkbox “Map in Foundry export” + `FeatureActiveEffectEditor` con pestañas Details/Duration/Changes/Auras alineadas a la hoja Foundry + DAE/Active Auras, preview JSON del AE vía `buildWeaponActiveEffect` / `weapon-effect.utils.ts` + panel de cadenas en `WeaponRarityEditor`).

**Catálogo Amellwind `/weapons`**: export Foundry por rareza desde el diálogo (`WeaponCatalogExportMenu` → mismo compilador; sin UI de edición de automation). Diálogos de arma (Amellwind + Forge) y el form de Forge incluyen un tab secundario **Foundry VTT** (`WeaponFoundryPreviewPanel`). El padre construye **un** `FoundryItem` (`buildAmellwindWeaponFoundryItem` en catálogo, `buildWeaponFoundryItem` en Forge) y ese mismo objeto alimenta el preview y la descarga Export (paridad payload). El panel refleja DETAILS + Activity (Identity / Activation / Targeting / Effect / Midi-QOL) + **Active Effects** (Details / Duration / Changes / Auras + flags DAE/Active Auras) leyendo solo campos del item; con `includeBase` el daño mostrado viene de `system.damage.base` del mismo JSON. El export escribe `range.reach` (5 ft, o 10 con `rch`) e `midiProperties.identifier: "attack"` en el ataque principal por defecto.
...
**Contrato JSON + módulos**: el JSON embebe schema dnd5e (stats, items, activities, descriptions, AE) y **referencia** comportamiento de módulos (`midiProperties`, `flags.midi-qol.*` / `flags.dae.*`, nombres canónicos para CPR/AA, content links `@item[…]`). Sin esos módulos el actor importa “plano”; con el stack activo las referencias cobran vida. Núcleo compartido: `src/shared/foundry/` (`FOUNDRY_EXPORT_TARGET`, `downloadFoundryJson`, tipos, midi, enrichers, mappings, icons). Catálogo de módulos: `shared/foundry/module-requirements.ts` + UI `FoundryModuleRequirementsNotice`.

| Capa | En el JSON | Módulo destino |
| --- | --- | --- |
| Schema / activities | Embebido | dnd5e 4.4.4 |
| Midi workflow | `midiProperties`, AE midi flags, triggered activities | midi-qol **12.4.28+** + libWrapper + socketlib |
| DAE / expiry | `flags.dae.*`, duration | DAE + Times Up |
| Auras (v12) | `flags.ActiveAuras.*` on AE | Active Auras |
| Content links en texto | `@variantrule[…]`, `@item[…]`, `@spell[…]` | Plutonium (recommended) |
| Rolls clickeables | `[[/r 2d4+2]]`, enrichers dnd5e | Foundry core + dnd5e |
| Deep links | `<a href="https://5e.tools/…">` | Navegador |
| Premades / animaciones | Nombres EN + flags toolbox | CPR / AA + Sequencer + JB2A |

- **Description enrichers** (`shared/foundry/description.ts`): `toFoundryDescriptionHtml` convierte tags 5etools `{@variantrule Advantage|XPHB}` → `@variantrule[Advantage|XPHB]`, `{@dice 2d4+2}` / dados sueltos → `[[/r …]]`, `{@filter …}` → links 5e.tools. Helpers de layout: `foundryDividerHtml()` (`<hr>`), `foundryRarityTitleHtml()` (colores Foundry/WoW por rareza), `foundryFeatureCardHtml` / `foundryChatFeatureCardHtml` / `foundryUpgradeBlockHtml` (cards inline para cadenas de features de armas), `foundryActivationLeadHtml` + `formatFeatureBodyHtml` (lead PHB + listas `-`/`•`). **No** usa `parseFiveToolsMarkup` (que strippea tags para la UI de la app). Todas las descriptions de export (feats, spells, weapons, runes, …) pasan por este pipeline.
...
- **Item / spell / feat images** (`foundry-icons.ts` + fluff maps en `fluff-lookup.ts`): `img` de spells/dotes usa arte 5etools cuando existe; si no, fallback por escuela (spells) o tipo. **Armas D&D** prefieren iconos core de Foundry por nombre base (p. ej. spear → `spear-flared-green.webp`) frente al fluff de libro (recorta mal en thumbnails). **Inventario/loot** sin fluff resuelve por tipo/nombre (`A`/bolts/arrows → munición, quiver → carcaj, GS/gaming set → dados, packs, etc.) en vez del saco genérico. Armas Amellwind siguen con su icono/`mh-icons` cuando aplica.
- **Weapon Mastery (D&D 5e)** (`weapon-mastery.data.ts` + `buildWeaponItem`): en armas `contentSource: "dnd"` se rellena `system.mastery` (clave Foundry, p. ej. `sap`) y se añade a `system.description` el bloque ***Mastery: Sap.*** + texto XPHB (tags `{@…}` → enrichers). Las armas Amellwind no reciben este apéndice (ya traen descripción propia). Las picks de Weapon Mastery del builder van a `traits.weaponProf.mastery.value` (baseItem slugs).
