...
- **Description enrichers** (`shared/foundry/description.ts`): `toFoundryDescriptionHtml` convierte tags 5etools `{@variantrule Advantage|XPHB}` → `@variantrule[Advantage|XPHB]`, `{@dice 2d4+2}` / dados sueltos → `[[/r …]]`, `{@filter …}` → links 5e.tools. Helpers `foundryDividerHtml()` (`<hr>`) y `foundryRarityTitleHtml()` (colores Foundry/WoW por rareza) para layouts de features en Weapon Forge. **No** usa `parseFiveToolsMarkup` (que strippea tags para la UI de la app). Todas las descriptions de export (feats, spells, weapons, runes, …) pasan por este pipeline.
