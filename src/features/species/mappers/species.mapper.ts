...
function mapSizes(size: unknown): SpeciesSize[] {
  if (!Array.isArray(size)) return ["Medium"];
  return size.map((s) => (SIZE_MAP[String(s)] ?? String(s)) as SpeciesSize);
}
...
function formatSpeed(speed: unknown): string {
  ...
  const parts: string[] = [];
  if (typeof s.walk === "number") parts.push(`walk ${s.walk} ft.`);
...
function mapResistances(resist: unknown): {
  fixed: DamageType[];
  summary: string;
} {
