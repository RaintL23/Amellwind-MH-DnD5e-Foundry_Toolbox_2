...
export function HuntPrepTablesPanel({ hunt }: HuntPrepTablesPanelProps) {
  ...
  return (
    <div className="space-y-4">
        ...
        <CardHeader className="flex flex-row flex-wrap items-start justify-between gap-3 space-y-0 p-4 pb-3">
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <CardDescription>
              ...
              Difficulty: {hunt.encounterDifficulty}. Collapse a section anytime
              to save space.
            </CardDescription>
          </div>
          <Button
      ...
      <Accordion
        type="multiple"
        defaultValue={DEFAULT_OPEN_TABLES}
        ...
        {HUNT_PREP_TABLE_META.map((meta) => {
          ...
          return (
            <AccordionItem
              ...
              value={meta.key}
              className={cn(
                "self-start rounded-lg border shadow-none",
                TABLE_ACCENT[meta.key],
                TABLE_BG[meta.key],
              )}
            >
              <div className="flex w-full items-center gap-1 pl-3 pr-2 py-2 [&>h3]:min-w-0 [&>h3]:flex-1">
                <AccordionTrigger className="gap-2 py-1.5 text-left hover:no-underline">
