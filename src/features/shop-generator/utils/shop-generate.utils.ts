...
function matchesTypePreference(
  typeLabel: string,
  preferred: string[],
): boolean {
...
function hasKeyword(item: DndItem, keywords: string[]): boolean {
...
export function filterShopPool(
  ...
  if (config.classAffinities.length > 0) {
    const profiles = config.classAffinities
    ...
    if (profiles.length > 0) {
      pool = pool.filter((item) =>
        profiles.some(
          (profile) =>
            matchesTypePreference(item.typeLabel, profile.preferredTypes) ||
            hasKeyword(item, profile.keywords),
        ),
      );
    }
  }
...
function scoreItem(item: DndItem, config: ShopConfig): number {
  ...
  if (matchesTypePreference(item.typeLabel, theme.preferredTypes)) {
    weight *= theme.weightBoost;
  }
  if (hasKeyword(item, theme.keywords)) {
    weight *= 1.8;
  }
  ...
  for (const classId of config.classAffinities) {
    const profile = getClassAffinity(classId);
    if (!profile) continue;
    if (matchesTypePreference(item.typeLabel, profile.preferredTypes)) {
      weight *= 1.6;
    }
    if (hasKeyword(item, profile.keywords)) {
      weight *= 1.4;
    }
  }
...
function weightedSample(
  ...
  while (picked.length < count && remaining.length > 0) {
    ...
    const key = chosen.item.name.toLowerCase();
...
function toStockEntry(item: DndItem): ShopStockEntry {
...
export async function generateShop(
  ...
  const selected = weightedSample(pool, config, config.itemCount);
  ...
  return {
    ...
    shopkeeper,
    stock: selected.map(toStockEntry),
    generatedAt: new Date().toISOString(),
  };
}

export function replaceStockEntry(
  items: DndItem[],
  config: ShopConfig,
  shop: GeneratedShop,
  rowId: string,
  ...
  const pool = filterShopPool(items, config).filter(
    (item) => !existingNames.has(item.name.toLowerCase()),
  );
  const [replacement] = weightedSample(pool, config, 1);
  if (!replacement) return shop;

  return {
    ...shop,
    stock: shop.stock.map((entry) =>
      entry.rowId === rowId ? toStockEntry(replacement) : entry,
    ),
  };
}
