 ...
 *
 * EMBEDS:
 * 1) Magazine consumable — Load Magazine (BA)
 *    On Use: [preTargeting]ItemMacro,[postActiveEffects]ItemMacro
 * 2) Dual Repeaters weapon
 *    On Use: [preTargeting]ItemMacro,[postAttackRoll]ItemMacro,[postDamageRoll]ItemMacro
 *    Magazines / Empowered Reload: dialog → expend magazine → fill weapon Charges (6)
 *    Attack: consumes 1 weapon Charge (item uses); AE tracks loaded magazine type
 *
 * Charges (bullets) live on the weapon sheet: system.uses max 6 (Bow-style resource).
 * Magazine AE only tracks which magazine is loaded (damage type), not the count.
 */

export const DUAL_REPEATERS_MAGAZINES_ITEM_MACRO = `// Dual Repeaters — Item Macros (MidiQOL 12.4 / Foundry v12 / dnd5e 4.4)
//
// EMBEDS:
