 ...
 *
 * Silkbind Tether:
 *   Places a radius MeasuredTemplate (15 ft Rare / 10 ft VR+) at the target,
 *   stamps the Tethered AE with template + DC metadata, and grants the target a
 *   temporary "Snap Tether" feat (STR save) to break free.
 *
 * Silkbind Grapple:
 *   Confirms the target is Tethered; Grappled status comes from the activity AE.
 *
 * Snap Tether (on this weapon):
 *   Hunter voluntarily releases the tether — no save; clears AE + template +
 *   the target's temporary Snap Tether feat.
 */
export const WIRE_KNUCKLES_SILKBIND_ITEM_MACRO = `// Wire Knuckles — Silkbind Item Macro (MidiQOL 12.4 / Foundry v12 / dnd5e 4.4)
// On Use: [postActiveEffects]ItemMacro
//
