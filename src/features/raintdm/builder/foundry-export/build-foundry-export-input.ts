...
import type { FeatureInput, FoundryExportInput } from "./actor-export.types";
import { renderFiveToolsEntries } from "./fluff-description";
import { gatherBuilderSnapshot } from "./gather-builder-snapshot";
import type { CartItemKind } from "../utils/cart-equipment.resolver";
import type { InventoryCatalogMeta, SpellItemInput } from "./item.builders";
...
/** Inventory slice used when assembling Foundry export input. */
export interface FoundryExportInventorySource {
  items: CartEntry[];
  weapons: Weapon[];
  armors: ArmorItem[];
  trinkets: string[];
...
/** Builder-derived state required to build sync Foundry export input. */
export interface BuildFoundryExportInputContext {
  builder: CharacterBuilderContextValue;
  inventory: FoundryExportInventorySource;
...
/**
 * Builds the sync portion of {@link FoundryExportInput} from current builder state.
 * Async enrichment (fluff, spells, item images) is applied later in export flow.
 */
export function buildFoundryExportInput(
  ctx: BuildFoundryExportInputContext,
  ...
  const useBackground =
    backgroundData && builder.background?.id === backgroundData.id
      ? backgroundData
      : null;
