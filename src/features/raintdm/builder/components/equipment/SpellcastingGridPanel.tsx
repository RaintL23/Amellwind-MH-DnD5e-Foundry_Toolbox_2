...
function getBonusCantripPoolEquipped(
  spellSelections: BuilderSpellSelections,
  pool: import("@/features/raintdm/builder/hooks/useSpellcasting").CantripPoolInfo,
  spellsByName: Spell[],
): { name: string; detail?: string } | null {
...
function getPactPoolEquipped(
  ...
  if (
    ...
    summaryParts.push(
        `${spellcastingInfo.pactSlotCount} slot${spellcastingInfo.pactSlotCount !== 1 ? "s" : ""} (lvl ${spellcastingInfo.pactMaxSpellLevel})`,
    );
  }
  ...
  return (
    <div
      className={cn(
        className,
        highlighted &&
          "rounded-md border border-amber-500/60 bg-amber-500/10 p-2 ring-1 ring-amber-500/30",
      )}
    >
