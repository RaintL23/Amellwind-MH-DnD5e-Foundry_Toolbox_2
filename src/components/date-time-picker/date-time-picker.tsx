...
export const DateTimePicker = ({
  ...
  const dislayFormat = useMemo(() => {
    if (!displayValue) return "Seleecione una fecha";
    return format(
      displayValue,
      `dd/MM/yyyy${!hideTime ? (use12HourFormat ? " hh:mm:ss a" : " HH:mm:ss") : ""}`,
    );
          ...
          <div className="text-md font-bold ms-2 flex items-center cursor-pointer">
            <div className="flex items-center">
              <div
                ...
                className={cn(
                  "flex items-center",
                  monthYearPicker == "year" ? "hidden" : "",
                )}
              >
                <span className="ms-1">
                  {safeCapitalize(format(month, "MMMM", { locale: es }))}
                </span>
                <Button
                  variant="ghost"
                  size="icon"
              ...
              <div
                ...
                className={cn(
                  "flex items-center",
                  monthYearPicker == "month" ? "hidden" : "",
                )}
              >
                <span className="ms-1">{format(month, "yyyy")}</span>
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() =>
...
const MonthYearPicker = ({
  ...
  const months = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      months.push({
        value: i,
        label: safeCapitalize(format(new Date(0, i), "MMM", { locale: es })),
        disabled,
      });
    }
    return months;
...
const buildTime = (options: BuildTimeOptions) => {
